...
const SectionMagazine8: FC<SectionMagazine8Props> = ({
	...
	return (
		<div className={`nc-SectionMagazine8 relative ${className}`}>
			...
			<div className={`grid grid-cols-1 gap-6 sm:grid-cols-6 md:gap-8`}>
				<Card16Podcast
				...
				<Card16Podcast
					...
					post={posts[1]}
				/>
				<div className="flex flex-col space-y-6 sm:col-span-6 md:space-y-8 lg:col-span-2">
					{posts
						.filter((_, i) => i > 1 && i < 6)
						.map((p) => (
							<Card17Podcast key={p._id} post={p} />
						))}
				</div>
			</div>
		</div>
