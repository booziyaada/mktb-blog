...
const SectionGridPosts: FC<SectionGridPostsProps> = ({
	...
	const renderCard = (post: PostDataType) => {
		switch (postCardName) {
			...
			case 'card4':
				return <Card4 key={post._id} post={post} />
			case 'card7':
				return (
					<Card7 key={post._id} post={post} ratio="aspect-w-5 aspect-h-5" />
				)
			case 'card9':
				return <Card9 key={post._id} post={post} />
			case 'card10':
			...
			case 'card14':
				return <Card14 key={post._id} post={post} />
			case 'card15Podcast':
				return <Card15Podcast key={post._id} post={post} />
			default:
				return null
		}
	}
